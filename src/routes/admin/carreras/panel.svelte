<script>
	...
	const clearCareer = ()=>{
		...
		divisionElement.className = elementClassSelect
		divisionFeedback = []
		levelElement.value = ""
		levelElement.className = elementClassSelect
		levelFeedback = []
	}
	...
	const clearOldCareer = ()=>{
		...
		olddivisionElement.className = elementClassSelect
		olddivisionFeedback = []
		oldlevelElement.value = ""
		oldlevelElement.className = elementClassSelect
		oldlevelFeedback = []
	}
	...
	const getLevels = async ()=>{
        axiosapi.doGet("/academic/level/get/active").then(res=>{
            levels = res.data;
        }).catch(()=>{
			swal.err()
        })
	...
	const getCareersBySearch = ()=>{
		searching = true
		let endpoint = '/career/get'
		if(careers.search != undefined && careers.search != ""){
			endpoint = '/career/get?search=' + careers.search
			searched = true
			wordSearched = careers.search
		}else{
			searched = false
		}
	...
	const getCareersByPage = (page) => {
		let endpoint = '/career/get?page=' + page
		if(searched){
	...
	const getCareersByNextPage = () => {
		getCareersByPage(careers.page + 1)
	}

	const getCareersByPreviousPage = () => {
		getCareersByPage(careers.page - 1)
	}
	...
	const updateCareer = ()=>{
		let id = oldcareer.id
		axiosapi.doPut("/career/update/"+id,oldcareer).then(()=>{
	...
	const createCareer = ()=>{
		axiosapi.doPost("/career/create",newcareer).then(()=>{
    ...
    const deleteCareer = (id)=>{
		swal.concan('question', TITQDELETE).then((result) => {
			if(result.isConfirmed){
				axiosapi.doDelete("/career/delete/"+id).then(()=>{
					getCareers()
	...
	const validAcronym = (target) => {
		...
		if(!value || value == ' '){
			validated = false
			target.className = elementClass + ' is-invalid'
			acronymFeedback.push("Completar el campo.")
		}
		if(!onlyLetters(value)){
			validated = false
			target.className = elementClass + ' is-invalid'
	...
	const validDivision = (target) => {
		...
		if(!value){
			validated = false
			target.className = elementClass + ' is-invalid'
			divisionFeedback.push("Seleccione una división académica.")
		}
		return validated
	}
	...
	const validLevel = (target) => {
		...
		if(!value){
			validated = false
			target.className = elementClass + ' is-invalid'
			levelFeedback.push("Seleccione un nivel académico.")
		}
		return validated
	}
	...
	const validoldName = (target) => {
		...
		if(!value || value == ' '){
			validated = false
			target.className = elementClass + ' is-invalid'
			oldnameFeedback.push("Completar el campo.")
		}
		if(!onlyLetters(value)){
			validated = false
			target.className = elementClass + ' is-invalid'
	...
	const validoldAcronym = (target) => {
		...
		if(!value || value == ' '){
			validated = false
			target.className = elementClass + ' is-invalid'
			oldacronymFeedback.push("Completar el campo.")
		}
		if(!onlyLetters(value)){
			validated = false
			target.className = elementClass + ' is-invalid'
			oldacronymFeedback.push("Introduzca solo letras.")
		}
		if(value.length > 10){
			validated = false
			target.className = elementClass + ' is-invalid'
			oldacronymFeedback.push("El acrónimo debe tener máximo 10 caracteres.")
		}
		return validated
	}
	...
	const validoldDivision = (target) => {
		...
		if(!value){
			validated = false
			target.className = elementClass + ' is-invalid'
			olddivisionFeedback.push("Seleccione una división académica.")
		}
		return validated
	}
	...
	const validoldLevel = (target) => {
		...
		if(!value){
			validated = false
			target.className = elementClass + ' is-invalid'
			oldlevelFeedback.push("Seleccione un nivel académico.")
		}
		return validated
	}

	const listenerValidity = () => {
		nameElement.addEventListener('input', (e) => {validName(e.target)})
		acronymElement.addEventListener('input', (e) => {validAcronym(e.target)})
		divisionElement.addEventListener('change', (e) => {validDivision(e.target)})
		levelElement.addEventListener('change', (e) => {validLevel(e.target)})
		oldnameElement.addEventListener('input', (e) => {validoldName(e.target)})
		oldacronymElement.addEventListener('input', (e) => {validoldAcronym(e.target)})
		olddivisionElement.addEventListener('change', (e) => {validoldDivision(e.target)})
		oldlevelElement.addEventListener('change', (e) => {validoldLevel(e.target)})
	}

	const checkValidationC = () => {
		let ok = true
		ok = validName(nameElement) && ok
		ok = validAcronym(acronymElement) && ok
	...
	const checkValidationU = () => {
		let ok = true
		ok = validoldName(oldnameElement) && ok
		ok = validoldAcronym(oldacronymElement) && ok
		ok = validoldDivision(olddivisionElement) && ok
		ok = validoldLevel(oldlevelElement) && ok
...
<main>
					...
					<div class="row justify-content-end">
						<div class="col-xs-12 col-sm-12 col-md-8 col-lg-8 col-xl-6 col-xxl-6">
							<div class="input-group mb-3">
								{#if searched}
									<button on:click="{()=>getCareers()}" class="btn btn-outline-success">
										<i class="fas fa-sync-alt"></i> Mostrar todos
									</button>
								{/if}
								<input on:keyup="{(e)=>{if(e.keyCode === 13){getCareersBySearch()}}}" 
									bind:value="{careers.search}" 
									type="search" 
									class="form-control" 
									placeholder="Busca algo..." 
									aria-label="Buscador" 
									aria-describedby="button-search">
									<button  on:click="{()=>getCareersBySearch()}" class="btn btn-outline-primary" type="button" id="button-search">
									{#if searching}
			...
			<div class="modal-content">
				<form on:submit="{(e) => {e.preventDefault(); checkValidationC();}}">
					...
					<div class="modal-footer">
						<button bind:this="{closemodalcreate}" on:click="{()=>{clearCareer()}}" type="button" class="btn btn-secondary" data-bs-dismiss="modal"><i class="fas fa-times"></i> Cancelar</button>
	...
	<div class="modal fade" id="mo2" tabindex="-1" aria-labelledby="mol2" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<form on:submit="{(e)=>{e.preventDefault();checkValidationU();}}">
					<div class="modal-header">
					...
					<div class="modal-footer">
						<button bind:this="{closemodalupdate}" on:click="{()=>{clearOldCareer()}}" type="button" class="btn btn-secondary" data-bs-dismiss="modal"><i class="fas fa-times"></i> Cancelar</button>
						<button type="submit" class="btn btn-primary" ><i class="fas fa-save"></i> Guardar</button>
